...
ALTER TABLE "vehicle_requests"
  ...
  -- PAY-D (WF:2297) names a `vehicle_requests` mirror of the deposit disclosure pair.
  ADD COLUMN IF NOT EXISTS "disclosures_accepted_at"           TIMESTAMP(3),
...
ALTER TABLE "deposits"
  ...
  -- PAY-38a (WF:2232) completes the dispute/refund hold triple. `disputed_at` alone records that a
  -- dispute opened; the hold is only legible with the reason it was placed and the moment it lifted.
  -- The Phase 3 predicate B3/V18 call `dispute_hold_at` is derived, not stored:
  --   on hold  ==  "disputed_at" IS NOT NULL AND "hold_released_at" IS NULL
  -- A stored fourth column would be a second spelling of that fact and could disagree with it.
  ADD COLUMN IF NOT EXISTS "hold_reason"              TEXT,
  ADD COLUMN IF NOT EXISTS "hold_released_at"         TIMESTAMP(3),
  -- PAY-D (WF:2297), written by PAY-08 in Phase 3.
  ADD COLUMN IF NOT EXISTS "disclosures_accepted_at"  TIMESTAMP(3),
  ADD COLUMN IF NOT EXISTS "disclosures_version"      TEXT;
...
ALTER TABLE "deals"
  ...
  ADD COLUMN IF NOT EXISTS "hold_reason"                           TEXT,
  -- PAY-46a (WF:2241) — the concierge-fee refund reason, the deal-side half of the pair whose
  -- deposit-side half is `deposits.refund_reason`.
  ADD COLUMN IF NOT EXISTS "fee_refund_reason"                     "RefundReason";
...
ALTER TABLE "offers"
  ...
  -- A4a (WF:2482) names the pair; A4b (Phase 6) stamps the timestamp on submit.
  ADD COLUMN IF NOT EXISTS "availability_confirmed_at"             TIMESTAMP(3),
...
-- ── shortlist_items: candidate distance (R43a WF:2120) ─────────────────────────────────────────
-- R43a puts the distance column and the inventory key in this wave alongside the cap trigger; only
-- the trigger had landed. The FK is in the section 6 list below.
...
ALTER TABLE "refinance_applications"
  ...
  ADD COLUMN IF NOT EXISTS "consent_ip"           TEXT,
  ADD COLUMN IF NOT EXISTS "consent_surface"      TEXT,
  -- §7 requires the controlled companion beside EVERY captured address. `consent_ip` is a captured
  -- address, so it takes the same pair and the same two CHECKs as the other four consent-bearing
  -- rows; without it this table is the one place a caller could reach for the `unavailable:<reason>`
  -- sentinel the constraint block below exists to make impossible. (`refinance_applications` has no
  -- `ip_address` column, so only the consent_ip half applies here.)
  ADD COLUMN IF NOT EXISTS "consent_ip_unavailable_reason" TEXT;
...
BEGIN
  FOR r IN
    SELECT * FROM (VALUES
      ...
      ('refinance_applications','consent_ip','consent_ip_unavailable_reason')
...
BEGIN
  FOR fk IN
    SELECT * FROM (VALUES
      ...
      -- R3 (WF:1542). `buyer_opportunity_id` already exists as a bare text column carrying the
      -- Lane-1 attribution link. R3 offers "add the guarded FK, or drop the @relation to a
      -- documented soft link"; the FK is the option that makes the lineage real, and preflight.sql
      -- asserts there is no orphan before it is added.
      ('vehicle_requests_buyer_opportunity_id_fkey', 'vehicle_requests',      'buyer_opportunity_id', 'buyer_opportunities','SET NULL'),
      -- R45 / R70 / U3. The document cannot exist without its pre-approval, so the child dies with
      -- the parent (R45 offers Cascade or Restrict; these are artefacts of the parent, not records
      -- with independent legal meaning, and Cascade is what the buyer-deletion path needs).
      ('external_pre_approval_documents_pre_approval_id_fkey','external_pre_approval_documents','pre_approval_id','external_pre_approvals','CASCADE'),
      -- R59 / U3. RESTRICT as specified: a Deal's status history is the audit trail of a legally
      -- retained record and must not vanish with it. This does NOT block account deletion — the
      -- buyer route (`app/api/buyer/account/route.ts:51-58`) never deletes a Deal; when one exists
      -- it anonymises the buyer and keeps the row.
      ('deal_status_history_deal_id_fkey',           'deal_status_history',   'deal_id',              'deals',             'RESTRICT'),
      -- R43a (WF:2120). The row names "SET NULL vs RESTRICT" but `inventory_item_id` is NOT NULL in
      -- production, so SET NULL is not available: a shortlisted item must keep pointing at a real
      -- listing, and the listing may not be deleted out from under a buyer's shortlist.
      ('shortlist_items_inventory_item_id_fkey',     'shortlist_items',       'inventory_item_id',    'inventory_items',   'RESTRICT'),
      -- deposits / auctions
...
-- the parent. The trigger below enforces that.
--
-- REFERENTIAL ACTIONS, and why both of them are spelled out so precisely:
--
--   * `ON DELETE SET NULL ("current_plan_snapshot_id")` names its column. A bare `SET NULL` on a
--     COMPOSITE key nulls EVERY referencing column, and the first one here is the table's own
--     PRIMARY KEY — so deleting any governing snapshot would run
--     `UPDATE vehicle_requests SET id = NULL, current_plan_snapshot_id = NULL` and fail 23502
--     against the not-null PK. The column list (PostgreSQL 15+, production runs 17.6) nulls only
--     the pointer, which is the whole intent.
--
--   * `ON UPDATE NO ACTION`, never CASCADE: a cascade on this key would propagate into
--     `vehicle_requests.id` / `deals.id` — rewriting a primary key is never what is wanted, and the
--     lineage SET NULL below makes the referenced key change for real.
--
-- Both were proven by execution against a 17.6 restore of the production baseline, not reasoned
-- about; `verify.sql` now deletes a snapshot, a request and a deal inside a rolled-back transaction
-- so this class of defect cannot pass an existence-only check again.
-- ════════════════════════════════════════════════════════════════════════════════════════════════
-- The REFERENCING side of each composite key. `id` is the primary key, so the pair is unique by
-- construction and these indexes add no rule production does not already enforce. They exist because
-- Prisma requires the defining side of a 1-1 composite relation to carry a matching unique
-- constraint, and without them `schema.prisma` and the migration chain disagree - which the drift
-- gate reports as a schema the chain cannot build.
CREATE UNIQUE INDEX IF NOT EXISTS "deals_id_current_plan_snapshot_id_key"
  ON "deals" ("id", "current_plan_snapshot_id");
CREATE UNIQUE INDEX IF NOT EXISTS "vehicle_requests_id_current_plan_snapshot_id_key"
  ON "vehicle_requests" ("id", "current_plan_snapshot_id");
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'vehicle_requests_current_plan_snapshot_fkey') THEN
    ALTER TABLE "vehicle_requests"
      ...
      ON DELETE SET NULL ("current_plan_snapshot_id") ON UPDATE NO ACTION;
  END IF;
...
-- The trigger refuses every edit EXCEPT the one PostgreSQL itself issues: `ON DELETE SET NULL` on
-- the lineage keys below is implemented as an UPDATE on this table, so an unconditional RAISE here
-- does not merely guard the snapshot — it makes every `vehicle_request` and every `deal` that has a
-- snapshot permanently undeletable, and takes the live buyer account-deletion route with it
-- (`app/api/buyer/account/route.ts:63` runs `vehicleRequest.deleteMany` unconditionally inside a
-- transaction). Verified by execution, not inferred.
--
-- So the one permitted update is the referential nulling of `vehicle_request_id` / `deal_id`: each
-- may go to NULL, neither may be given a new value, and every other column must be byte-identical.
-- The jsonb comparison is column-list-agnostic on purpose — a column added to this table by a later
-- migration is covered by the immutability rule automatically instead of silently escaping it.
CREATE OR REPLACE FUNCTION "plan_snapshots_append_only"() RETURNS trigger AS $fn$
BEGIN
  IF (NEW."vehicle_request_id" IS NOT DISTINCT FROM OLD."vehicle_request_id"
        OR NEW."vehicle_request_id" IS NULL)
     AND (NEW."deal_id" IS NOT DISTINCT FROM OLD."deal_id"
        OR NEW."deal_id" IS NULL)
     AND (to_jsonb(NEW) - 'vehicle_request_id' - 'deal_id')
       = (to_jsonb(OLD) - 'vehicle_request_id' - 'deal_id')
  THEN
    RETURN NEW;
  END IF;
...
-- ── R30 (WF:1574) — a Deal must be reachable from an offer ─────────────────────────────────────
-- R30's defect statement is that a Deal cannot identify its vehicle/VIN/dealership/auction/request.
-- R31-R35 add the lineage columns; this CHECK is what stops a new Deal being written with no offer
-- lineage at all. Production held 0 `deals` rows when read on 2026-09-05, but that is a MEASUREMENT
-- taken before the deploy, not a property of the table — preflight.sql re-asserts it at deploy time
-- against the data that will actually be there, exactly as the `assigned_admin_id` key does above.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'deals_offer_lineage_check') THEN
    ALTER TABLE "deals" ADD CONSTRAINT "deals_offer_lineage_check"
      CHECK ("offer_id" IS NOT NULL OR "vehicle_request_offer_id" IS NOT NULL);
  END IF;
END $$;
