// The Phase 9 proof SQL must name EVERY Phase 9 migration — checked against the directories on
...
// HALF THE MIGRATIONS MISSING. Phase 9 ships ONE migration, which makes the drift risk feel
// theoretical — it is not. A one-name list is exactly the list nobody updates when a second
// migration is added late, which is precisely how phase 8's happened.
//
...
// `20261201` is 8 digits and the stamp is 14, so the pattern below takes `\d{6}` more. The
// anti-vacuity assertions are what prove that arithmetic rather than asserting it.
//
// Run: npx tsx --test "prisma/__tests__/phase9-proof-sql.test.ts"
...
const PROOF_DIR = join(REPO_ROOT, "docs", "transaction-flow", "phase-9-proof");

const PROOF_FILES = ["preflight.sql", "verify.sql"] as const;

/** The one migration this phase is known to ship. Named so a regex that matched some OTHER
 *  directory could not satisfy the anti-vacuity floor below on its own. */
const KNOWN = "20261201000000_phase9_pickup_release_token";

/** Every Phase 9 migration directory, from disk. 8-digit date + 6-digit time = the 14-digit stamp. */
function phase9MigrationsOnDisk(): string[] {
  return readdirSync(MIGRATIONS_DIR)
    .filter((d) => /^20261201\d{6}_phase9_/.test(d))
    .sort();
}
...
function migrationsNamedIn(file: string): string[] {
  ...
  return [...new Set([...sql.matchAll(/'(20261201\d{6}_phase9_[a-z0-9_]+)'/g)].map((m) => m[1]!))].sort();
}

describe("phase 9 proof SQL names every phase 9 migration", () => {
  test("the migrations exist on disk at all — the fixture is not empty", () => {
    const found = phase9MigrationsOnDisk();
    ...
    assert.ok(found.length >= 1, `expected at least 1 phase 9 migration, found ${found.length}`);
    assert.ok(
      found.includes(KNOWN),
      `the known phase 9 migration ${KNOWN} is not in ${JSON.stringify(found)} — the prefix ` +
        `pattern matches the wrong set, which is how a drift guard silently guards nothing`,
    );
  });

  for (const file of PROOF_FILES) {
    test(`${file} names every phase 9 migration on disk`, () => {
      const onDisk = phase9MigrationsOnDisk();
      ...
      assert.ok(named.length >= 1, `${file} names no phase 9 migration at all — the scan is broken, not the file`);
      const missing = onDisk.filter((m) => !named.includes(m));
      assert.deepEqual(
        missing,
        [],
        `${file} does not name ${missing.join(", ")}. A proof file that omits a migration ` +
          `reports a clean deploy with that migration missing — add it to the phase9_expected list.`,
      );
    });
    ...
    test(`${file} states no hardcoded count of phase 9 migrations`, () => {
      ...
      // applied row PER phase 9 migration" is the per-migration idiom: it stays true at one
      ...
      assert.deepEqual(
        ...
        `${file} hardcodes a migration count (${literals.join(", ")}). Derive it from the ` +
          `phase9_expected list instead — a literal beside a list is how phase 8's file drifted.`,
      );
    });
  }
  ...
  test("the proof files are all present", () => {
    for (const f of PROOF_FILES) {
      assert.ok(existsSync(join(PROOF_DIR, f)), `${f} is part of the phase 9 proof package and must exist`);
    }
  });
  ...
  // assertion, N per VALUES list, one per name in the phase9_expected CTE.
  ...
  function rowsEmitted(stmt: string): number {
    ...
    if (/FROM phase9_expected\s+AS\s+\w+/.test(stmt)) {
      return (stmt.match(/\('20261201\d{6}_phase9_/g) ?? []).length;
    }
    return 1;
  }
