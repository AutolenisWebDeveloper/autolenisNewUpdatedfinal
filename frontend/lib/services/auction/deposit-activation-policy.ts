...
export type ActivationAction =
  ...
  | 'refund'          // ACTIVE, zero invitations past the no-dealer grace — close → F-001 refund
...
export interface ActivationState {
  ...
  noDealerRefundGraceMinutes: number;
}

// What is the NEXT action to converge this deposit's activation toward the
// invariant: a confirmed $99 always reaches a populated ACTIVE auction OR a refund.
export function classifyActivation(s: ActivationState): ActivationAction {
  ...
  if (s.auctionStatus === 'ACTIVE') {
    ...
    if (s.auctionAgeMinutes >= s.noDealerRefundGraceMinutes) return 'refund';
    return 'invite';
  }
