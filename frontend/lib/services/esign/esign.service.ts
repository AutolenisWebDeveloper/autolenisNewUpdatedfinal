...
export async function sendEnvelope(dealId: string, signerKind: ESignSignerKind = "BUYER"): Promise<void> {
  ...
  const envelope = await prisma.eSignEnvelope.findUnique({ where: { dealId_signerKind: { dealId, signerKind } }, select: { id: true, status: true } });
  ...
  await prisma.eSignEnvelope.update({ where: { dealId_signerKind: { dealId, signerKind } }, data: { status: ESignStatus.SENT, sentAt: new Date() }, select: { id: true } });
...
//
// §13-D30: scoped to ONE signer. Voiding "the deal's envelope" is no longer a coherent
// instruction — an admin voiding the buyer's envelope must not silently void the
// co-buyer's completed signature, and vice versa.
export async function voidEnvelope(dealId: string, reason: string, signerKind: ESignSignerKind = "BUYER"): Promise<void> {
  const envelope = await prisma.eSignEnvelope.findUnique({ where: { dealId_signerKind: { dealId, signerKind } }, select: { id: true, status: true } });
...
export async function resendEnvelope(dealId: string, signerKind: ESignSignerKind = "BUYER"): Promise<void> {
  await sendEnvelope(dealId, signerKind);
}
