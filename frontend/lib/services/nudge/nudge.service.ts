...
export async function triggerNudge(buyerId: string, stage: NudgeStage): Promise<void> {
  ...
  const nudgeMessages: Record<NudgeStage, { title: string; body: string }> = {
    ...
    DEPOSIT_IDLE: { title: "Dealers are waiting", body: `Activate your auction with a ${DEPOSIT_AMOUNT_USD} Auction Access Fee — refundable if no valuable offer is received — and let dealers compete.` },
