// lib/services/email/templates/dealer-application-received.tsx
// Dealer Application Received — sent when a dealer application is submitted.
// Uses plain HTML string — no react-dom/server import (not compatible with Next.js App Router).

export const DEALER_APPLICATION_RECEIVED_SUBJECT =
  "Your Dealer Application Has Been Received — AutoLenis";

export interface DealerApplicationReceivedEmailProps {
  dealershipName: string;
  contactName: string;
}

export function renderDealerApplicationReceivedEmail({
  dealershipName,
  contactName,
}: DealerApplicationReceivedEmailProps): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Dealer Application Received — AutoLenis</title>
</head>
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:32px 0;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;max-width:600px;width:100%;">
          <!-- Header -->
          <tr>
            <td style="background:#0B5FD1;padding:32px;text-align:center;">
              <p style="color:#ffffff;font-size:26px;font-weight:bold;margin:0;letter-spacing:-0.5px;">AutoLenis</p>
              <p style="color:#DBEAFE;font-size:14px;margin:8px 0 0 0;">Dealer Network</p>
            </td>
          </tr>
          <!-- Body -->
          <tr>
            <td style="padding:40px 40px 24px 40px;color:#333333;font-size:15px;line-height:1.6;">
              <p style="margin:0 0 16px 0;">Hi ${contactName},</p>
              <p style="margin:0 0 16px 0;">Thank you for applying to join the AutoLenis dealer network. We have received your application for <strong>${dealershipName}</strong> and our team will review it shortly.</p>
              <p style="margin:0 0 24px 0;">Here is what to expect next:</p>
              <!-- Steps -->
              <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px 0;">
                <tr>
                  <td style="padding:8px 0;vertical-align:top;width:28px;">
                    <span style="display:inline-block;background:#0B5FD1;color:#ffffff;border-radius:50%;width:20px;height:20px;text-align:center;font-size:12px;font-weight:bold;line-height:20px;">1</span>
                  </td>
                  <td style="padding:8px 0;color:#555555;font-size:14px;line-height:1.5;">
                    <strong>Application review</strong> — our team reviews your submission within 1 business day.
                  </td>
                </tr>
                <tr>
                  <td style="padding:8px 0;vertical-align:top;width:28px;">
                    <span style="display:inline-block;background:#0B5FD1;color:#ffffff;border-radius:50%;width:20px;height:20px;text-align:center;font-size:12px;font-weight:bold;line-height:20px;">2</span>
                  </td>
                  <td style="padding:8px 0;color:#555555;font-size:14px;line-height:1.5;">
                    <strong>Approval email</strong> — if approved, you will receive an email with your invitation link to activate your account.
                  </td>
                </tr>
                <tr>
                  <td style="padding:8px 0;vertical-align:top;width:28px;">
                    <span style="display:inline-block;background:#0B5FD1;color:#ffffff;border-radius:50%;width:20px;height:20px;text-align:center;font-size:12px;font-weight:bold;line-height:20px;">3</span>
                  </td>
                  <td style="padding:8px 0;color:#555555;font-size:14px;line-height:1.5;">
                    <strong>Account onboarding</strong> — complete your dealership profile, upload inventory, and configure your DMS feed.
                  </td>
                </tr>
                <tr>
                  <td style="padding:8px 0;vertical-align:top;width:28px;">
                    <span style="display:inline-block;background:#0B5FD1;color:#ffffff;border-radius:50%;width:20px;height:20px;text-align:center;font-size:12px;font-weight:bold;line-height:20px;">4</span>
                  </td>
                  <td style="padding:8px 0;color:#555555;font-size:14px;line-height:1.5;">
                    <strong>Auction access</strong> — start receiving buyer auction invitations and submitting competitive offers.
                  </td>
                </tr>
              </table>
              <p style="margin:0;font-size:13px;color:#888888;">If you have any questions in the meantime, contact us at <a href="mailto:support@autolenis.com" style="color:#0B5FD1;text-decoration:none;">support@autolenis.com</a>.</p>
            </td>
          </tr>
          <!-- Footer -->
          <tr>
            <td style="background:#f8f8f8;padding:24px 40px;text-align:center;border-top:1px solid #F0F9FF;">
              <p style="margin:0;font-size:13px;color:#888888;">AutoLenis, Inc. &middot; <a href="mailto:support@autolenis.com" style="color:#0B5FD1;text-decoration:none;">support@autolenis.com</a></p>
              <p style="margin:4px 0 0 0;font-size:12px;color:#aaaaaa;"><a href="#" style="color:#aaaaaa;">Unsubscribe</a></p>
            </td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</body>
</html>`;
}
