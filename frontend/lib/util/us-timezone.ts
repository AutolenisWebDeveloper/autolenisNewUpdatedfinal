// Pure US state/ZIP → IANA timezone resolution.
//
// Extracted from lib/crm/recipient-timezone.ts so it can be shared by modules
// that must NOT pull the `server-only` marker (e.g. the pickup availability
// service, imported by both server routes and plain unit tests). The maps are
// the single source of truth; recipient-timezone.ts re-uses them for TCPA
// quiet-hours, and the pickup gate re-uses them to derive a dealer's timezone
// from its address. No side effects, no `server-only`.
...
// Coarse ZIP3 (leading-3-digit) → timezone. Used only when state is absent, or
// preferred over state when both are present (a ZIP3 pins a split-zone state).
// Ranges follow USPS sectional-center geography at a region granularity; any
// prefix not matched falls through to null (caller decides the fallback).
export function zip3ToTimezone(zip: string): string | null {
...
// Resolve a US IANA timezone from state + zip. ZIP3 is PREFERRED over state when
// both are present: a ZIP3 prefix pins the actual zone of a split-zone state
// (e.g. El Paso, TX is Mountain while the rest of TX is Central), whereas
// STATE_TZ only knows the state's dominant zone. State is the fallback when ZIP3
// doesn't resolve; null when neither does.
export function resolveUsTimezone(
  state?: string | null,
  zip?: string | null,
): string | null {
  if (zip) {
