...
export default async function AffiliateNetworkPage() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl" data-testid="affiliate-network-page">
      ...
      {/* Visual tree */}
      {l1Members.length === 0 ? (
        <div className="text-center py-16 bg-white border border-slate-200 rounded-xl" data-testid="empty-network">
          <GitBranch size={32} className="text-slate-300 mx-auto mb-3" />
          <p className="text-slate-500 mb-1">Your network is empty</p>
          <p className="text-xs text-slate-400">Share your referral link to start earning from L1, L2, and L3 commissions.</p>
        </div>
      ) : (
            ...
            <div key={l1.id} className="ml-8 border-l-2 border-[#0B5FD1]/20 pl-5 space-y-2" data-testid={`l1-member-${i}`}>
              <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">L1 · {l1.children.length} referrals</p>
                </div>
              ...
              {l1.children.slice(0, 3).map((l2, j) => (
                <div key={l2.id} className="ml-8 border-l-2 border-slate-200 pl-4 space-y-1.5" data-testid={`l2-member-${i}-${j}`}>
                  ...
                  {l2.children.slice(0, 2).map((l3, k) => (
                    <div key={l3.id} className="ml-6 border-l-2 border-slate-100 pl-3" data-testid={`l3-member-${i}-${j}-${k}`}>
                  ...
                  {l2.children.length > 2 && (
                    <p className="ml-6 pl-3 text-xs text-slate-400" data-testid={`l3-more-${i}-${j}`}>
              ...
              {l1.children.length > 3 && (
                <p className="ml-8 pl-4 text-xs text-slate-400" data-testid={`l2-more-${i}`}>
