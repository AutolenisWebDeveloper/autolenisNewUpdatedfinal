...
export default async function UnsubscribedPage({ searchParams }: { searchParams: Promise<{ status?: string; reason?: string }> }) {
  ...
  if (reason === "rejected") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-al-bg via-white to-al-primary-subtle flex items-center justify-center p-6" data-testid="affiliate-unsubscribed-page">
        <div className="w-full max-w-md bg-white border border-al-border rounded-2xl p-10 text-center shadow-sm">
          ...
          <h1 className="text-xl font-bold text-al-text mb-2 tracking-tight" data-testid="unsubscribed-title">
            Your affiliate application was not approved
          </h1>
          <p className="text-sm text-al-text-muted leading-relaxed mb-6" data-testid="unsubscribed-message">
          ...
          <div className="flex flex-col gap-2 text-sm">
            ...
            <Link href="/" className="text-al-text-subtle hover:text-al-primary transition-colors text-xs">Return to AutoLenis</Link>
          </div>
        </div>
      </div>
    );
  ...
  const config: Record<string, { icon: typeof CheckCircle2; title: string; message: string; tone: string }> = {
    success: {
      ...
      tone: "text-al-success bg-al-success-subtle border-al-success/30",
    },
    already: {
      icon: Info,
      title: "Already unsubscribed",
      message: "You had already opted out of the weekly digest. No change was made.",
      tone: "text-al-primary bg-al-primary/10 border-al-primary/30",
    },
    invalid: {
      icon: AlertCircle,
      title: "Invalid link",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-al-bg via-white to-al-primary-subtle flex items-center justify-center p-6" data-testid="affiliate-unsubscribed-page">
      <div className="w-full max-w-md bg-white border border-al-border rounded-2xl p-10 text-center shadow-sm">
        ...
        <h1 className="text-xl font-bold text-al-text mb-2 tracking-tight" data-testid="unsubscribed-title">{c.title}</h1>
        <p className="text-sm text-al-text-muted leading-relaxed mb-6" data-testid="unsubscribed-message">{c.message}</p>
        <div className="flex flex-col gap-2 text-sm">
