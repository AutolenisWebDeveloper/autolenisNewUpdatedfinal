        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-8 shadow-sm">
          ...
          {!loading && account && !success && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button data-testid="claim-submit" type="submit" disabled={submitting} className="w-full bg-[#0B5FD1] hover:bg-[#0A4DB8] disabled:opacity-50 text-white font-semibold py-3 rounded-xl transition-colors text-sm">{submitting ? "Setting Password..." : "Set Password & Continue"}</button>
            </form>
          )}
...
export default function DealerClaimPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#F8F9FB] flex items-center justify-center text-[#0B5FD1] text-sm">Loading...</div>}>
      <DealerClaimInner />
    </Suspense>
  );
}
