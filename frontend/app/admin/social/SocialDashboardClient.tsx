...
function OverviewTab({
  ...
  const generateForSignal = async (signal: Signal) => {
    ...
    try {
      // Default to the dealer_secret_daily franchise's first platform when not specified.
      const data = await fetchJson<{ created: string[] }>("/api/admin/social/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ signalId: signal.id, franchiseSlug: "how_autolenis_works" }),
      });
      showToast(`Generated ${data.created.length} post(s)`);
      await loadSignals();
      await onRefresh();
    } catch (err) {
