...
export async function POST(request: NextRequest) {
  ...
  // Persist the proof BEFORE releasing the gate. EXTERNAL_UPLOADED both advances
  // the deal and satisfies the final-release hard gate, so flipping that flag while
  // the uploaded file is referenced by nothing would mean the gate opens on evidence
  // no admin can retrieve or verify. InsurancePolicy is the existing home for
  // proofUrl — reused here rather than adding a column. If this write fails we do
  // NOT flip the status: no retrievable proof, no gate release.
  try {
    ...
    logger.error("[upload-proof] could not record the insurance proof:", err);
    return errorResponse(
      "PROOF_RECORD_FAILED",
      "We stored your file but couldn't record it against your deal. Please try again or contact support.",
      500,
    );
  }
