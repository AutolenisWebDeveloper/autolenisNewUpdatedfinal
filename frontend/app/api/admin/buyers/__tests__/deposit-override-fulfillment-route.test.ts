...
// FINANCE_ADMIN is inside PERMISSION_ROLES["finance.deposit.override"], so the
// gate below admits it and these fulfillment assertions get to run. Resolving
// that through roleAllows() rather than asserting it keeps this suite honest: if
// the matrix ever stopped admitting FINANCE_ADMIN, these tests would 403 rather
// than silently keep exercising a path the server no longer allows.
