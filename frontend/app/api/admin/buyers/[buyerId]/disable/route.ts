// POST /api/admin/buyers/[buyerId]/disable
// Disable a buyer's login access. Sets disabledAt timestamp.
// NOTE: Full Supabase auth ban requires Supabase Admin API (documented gap — not implemented here).
...
import { z } from "zod";
import { disableBuyerAccessByAdmin } from "@/lib/services/admin/admin-buyer-command-center.service";
...
export async function POST(request: NextRequest, { params }: Props) {
  ...
  try {
    const result = await disableBuyerAccessByAdmin(buyerId, admin.adminId, admin.email, parsed.data.reason);
    return adminSuccess(result);
  } catch (err) {
    return adminError("ACTION_FAILED", err instanceof Error ? err.message : "Disable failed", 400);
  }
}
