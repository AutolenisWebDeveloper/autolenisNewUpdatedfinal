// POST /api/admin/buyers/[buyerId]/archive
// Archive a buyer profile — removes from active lists, preserves all history.
...
import { z } from "zod";
import { archiveBuyerByAdmin } from "@/lib/services/admin/admin-buyer-command-center.service";
...
export async function POST(request: NextRequest, { params }: Props) {
  ...
  try {
    const result = await archiveBuyerByAdmin(buyerId, admin.adminId, admin.email, parsed.data.reason);
    return adminSuccess(result);
  } catch (err) {
    return adminError("ACTION_FAILED", err instanceof Error ? err.message : "Archive failed", 400);
  }
}
