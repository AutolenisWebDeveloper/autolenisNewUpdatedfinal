// POST /api/admin/buyers/[buyerId]/workflow/cancel
// Cancel an active deal for a buyer.
...
import { z } from "zod";
import { cancelBuyerWorkflow } from "@/lib/services/admin/admin-buyer-command-center.service";

interface Props { params: Promise<{ buyerId: string }> }

const schema = z.object({
  dealId: z.string().min(1, "Deal ID is required"),
  reason: z.string().min(1, "Reason is required for workflow cancellation"),
});

export async function POST(request: NextRequest, { params }: Props) {
  const { buyerId } = await params;
  // Tier 1 (Finding 5): enforced directly from PERMISSION_ROLES.
  // Cancels an active deal for a buyer.
  const adminCheck = await requirePermissionStrict(request, "deals.cancel");
  ...
  try {
    const result = await cancelBuyerWorkflow(
      buyerId,
      admin.adminId,
      admin.email,
      parsed.data.dealId,
