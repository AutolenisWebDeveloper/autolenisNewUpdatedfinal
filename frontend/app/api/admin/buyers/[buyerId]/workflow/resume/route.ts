// POST /api/admin/buyers/[buyerId]/workflow/resume
// Resume a paused (PENDING) auction for a buyer.
...
import { z } from "zod";
import { resumeBuyerWorkflow } from "@/lib/services/admin/admin-buyer-command-center.service";

interface Props { params: Promise<{ buyerId: string }> }

const schema = z.object({
  auctionId: z.string().min(1, "Auction ID is required"),
  reason: z.string().min(1, "Reason is required for workflow resume"),
});
...
export async function POST(request: NextRequest, { params }: Props) {
  ...
  try {
    const result = await resumeBuyerWorkflow(
      buyerId,
      admin.adminId,
      admin.email,
