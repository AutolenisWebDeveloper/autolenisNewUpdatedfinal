// POST /api/affiliate/ai/chat — Affiliate concierge (Zura)
// Groq ONLY | Kill switch | Affiliate auth required
import { NextRequest } from "next/server";
import { getRequestAffiliate, successResponse, errorResponse } from "@/lib/auth/affiliate-api";
import { affiliateConciergeChat } from "@/lib/services/ai/affiliate-concierge.agent";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await affiliateConciergeChat(
      affiliate.id,
      body.message,
