...
| GitGuardian flagged a hard-coded password literal in `form-walk.spec.ts` on the PR | **FIXED.** It was a throwaway value for one registration against the local database, not a real credential — but a credential-shaped literal in a committed file is one, and a scanner that waved it through would wave the next one through too. It is now derived from the run id: unique per run, satisfies the endpoint's complexity rule, no literal left to flag. |
