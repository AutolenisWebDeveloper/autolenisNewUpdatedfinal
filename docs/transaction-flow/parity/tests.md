...
3. CI (`.github/workflows/ci.yml`) has five jobs: `ci` (typecheck → lint → coverage-check → safety guards → sanitized DB-target report → `test:all` → build, placeholder DSN), `migrations` (empty **postgres 17.6** → major-version assertion → `prisma migrate deploy` twice → `db:check-drift` ratchet at 345 → 15 CRM runbook files twice), `e2e` (**postgres 17.6**, major-version assertion, the destructive concurrency suite against the ephemeral `autolenis_e2e`, then ONLY `tests/e2e/dealer-outreach.spec.ts`, admin JWT minted by `scripts/e2e-admin-storage-state.ts`), `phase1-proof` (**postgres 17.6**, restores the committed production physical-schema baseline, asserts the eight full-definition digests, then runs `docs/transaction-flow/phase-1-proof/run-proof.sh` end to end), and `dependency-audit` (blocks on critical). Every PostgreSQL service is pinned to production's major; major ≠ 17 fails the job, a patch level other than `170006` only warns. `visual.yml` is a separate, path-filtered workflow pinned to ubuntu-24.04.
