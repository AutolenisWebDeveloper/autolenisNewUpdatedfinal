...
| `test:concurrency` | `lib/services/deal/__tests__/select-offer-concurrency.test.ts` | 1 (skips without real DB) |
...
- No suite is marked skipped/quarantined in package.json or CI. Only two DB-gated tests self-skip: `lib/services/deal/__tests__/select-offer-concurrency.test.ts:23-25` (`skip: "no real DATABASE_URL — REQUIRES LIVE INFRASTRUCTURE"`) and `lib/services/ai/action-intent/__tests__/postgres-concurrency.test.ts:20` (`t.skip("NOT VERIFIED — set ACTION_INTENT_TEST_DATABASE_URL…")`). CI runs `test:all` with a placeholder DSN (`ci.yml:69-72`), so both skip in CI.
...
| S8 | Offer selection serialized under a DB lock | `lib/services/deal/__tests__/select-offer-concurrency.test.ts:23` — **skips without a real DB** | `:25` `skip: "no real DATABASE_URL — REQUIRES LIVE INFRASTRUCTURE"` | PROVEN only when run against Postgres; NOT VERIFIED in CI |
...
| Offer selection single winner | concurrency (real Postgres) | `lib/services/deal/__tests__/select-offer-concurrency.test.ts` | `test:concurrency` (needs CI DB job) |
...
3. **Row 53 — real-DB concurrency proofs run somewhere** | BROKEN in CI → **UNVERIFIED in CI + SAFETY HAZARD** | The claim "both DB-gated tests skip under the placeholder DSN" holds only if the GitHub secret `DATABASE_URL` is unset — CI uses `secrets.DATABASE_URL || placeholder`, and whether that secret exists was not (and could not be) verified here. `select-offer-concurrency.test.ts` is chained into `test:all` via `test:concurrency`, gates only on `!dsn.includes("placeholder")`, carries **no `autolenis_e2e` guard**, and in 110 lines creates users, buyers, deposits, auctions, dealers and offers with **no cleanup** (no `delete`/`deleteMany`/`truncate`/rollback anywhere in the file). If the secret is a real database, every CI run of `test:all` writes 5 rounds × 8 dealers of race fixtures into it. | `.github/workflows/ci.yml:70` `DATABASE_URL: ${{ secrets.DATABASE_URL || 'postgresql://placeholder:…' }}`; `lib/services/deal/__tests__/select-offer-concurrency.test.ts:20-21` `const hasRealDb = dsn.length > 0 && !dsn.includes("placeholder");`; `:39-61` `prisma.user.create` / `prisma.buyer.create` / `prisma.deposit.create` / `prisma.auction.create` / `prisma.dealer.create` / `prisma.offer.create`; `:32-33` `ROUNDS = 5; OFFERS_PER_AUCTION = 8`. **Required change:** add the same `/autolenis_e2e/` refusal the E2E specs and `scripts/e2e-admin-storage-state.ts:25-30` use (skip if unset, *throw* if set but not e2e), wrap each round in a transaction that rolls back or `deleteMany` in `finally`, and move `test:concurrency` out of `test:all` into the postgres-service CI job.
