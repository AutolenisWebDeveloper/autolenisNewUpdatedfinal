...
**OWNER DECISION (resolved):** **deposit-optional concierge.** The admin-sourced/concierge path converges into `Deal.offerId` **without** requiring a $99 deposit; `Auction.depositId` becomes nullable. The competitive path is unchanged (always deposit-backed).

## 23. Batch 4 — Canonical offer convergence (delivered on this branch)

**Scope delivered (TIER B/C · owner-gated decision resolved = deposit-optional).** Converged the concierge offer track onto the ONE canonical accepted-offer authority `Deal.offerId`, made the concierge auction deposit-optional, and fixed the buyer-facing fabricated-junk-fee defect (FS-I). Deliberately **left downstream money/contract/financing mechanics for Batch 6** — this batch wires Deal *creation* via the canonical path only.

**Schema (additive, owner-gated migration `20261011000000_batch4_canonical_offer_convergence`):**
- `Auction.depositId` → **nullable** (`String? @unique`), relation `deposit Deposit?` — a concierge auction can exist without a $99 deposit. Competitive auctions always pass a real deposit (unchanged). `ALTER TABLE "auctions" ALTER COLUMN "deposit_id" DROP NOT NULL;` — safe on existing rows.
- `Offer.conciergeSourceRef` → new `String? @unique @map("concierge_source_ref")` — the idempotency + provenance anchor. Unique index ignores NULLs (Postgres treats NULLs as distinct), so ordinary competitive offers (null ref) never collide. `ADD COLUMN IF NOT EXISTS` + `CREATE UNIQUE INDEX IF NOT EXISTS`.
- **PRODUCTION CUTOVER REMAINS OWNER-GATED** (`prisma migrate deploy`); verify physical schema, never applied to production in this batch.

**Delivered:**
- **`lib/services/offer/concierge-offer.service.ts`** (NEW) — `convertConciergeOfferToDeal(ctx)`: the single convergence path. Represents a concierge offer as the authoritative artifacts — a deposit-OPTIONAL, already-CLOSED `Auction` + a canonical `Offer` — and produces a `Deal` through `Deal.offerId`. **Idempotent + concurrency-safe** via `Offer.conciergeSourceRef @unique`: a repeat call (or a P2002 race) returns the existing/winner's `Deal`, minting nothing. **Honest pricing:** a single concierge OTD becomes `vehiclePrice=OTD, tax=0, fees=0, junkFeeItems=[]` — truthful, never fabricated. Resolves dealer identity to a registered `Dealer` when known, else the **Outside-Dealer placeholder** + `externalDealer*` fields. Validates `otdPriceCents > 0` and financing consistency (`aprRate`+`termMonths` required when `includesFinancing`).
- **System C converged** — `app/api/buyer/requests/[requestId]/offer/respond/route.ts`: buyer ACCEPT now calls `convertConciergeOfferToDeal({ sourceRef: "vehicle_request_offer:<id>", … })` instead of the legacy `deal.create({ vehicleRequestOfferId })`. The `Deal.vehicleRequestOfferId` write path is retired (column kept read-only for history). DECLINE path and graceful-degradation-on-failure path unchanged and still truthful.
...
- **`rankOffers`** now writes a best-effort `BestPriceCalculationLog` (weights + per-offer result) so the buyer-facing ranking is reproducible and auditable, not a black box. Fire-and-forget (`.catch(() => {})`) — a logging failure never affects ranking.
- **Nullable-`depositId` readers hardened** — `deposit-activation.service.ts` sweep now filters `depositId: { not: null }` (concierge auctions are never reconciled by the deposit-heal path) and narrows the type; `offer.service.ts` first-offer email guards the deposit lookup for a nullable id.

**System B (`VehicleOffer` concierge) — verified already honest; auto-conversion deferred to Batch 4b.** The `buyer-offer-review/[reviewToken]/respond` path was inspected end-to-end and is **truthful lead-capture, not fake-success**: it records the buyer's acceptance (`BuyerOfferReviewItem.status`), sets `VehicleOffer.requestStatus="buyer_interested"`, notifies admin + dealer, and the buyer-facing email/UI say only "We received your interest — our team will follow up" (no fabricated "deal complete", no dead redirect). Because System B carries **no authenticated `Buyer` FK** (public review token + `buyerEmail`), full auto-conversion into `Deal.offerId` needs a security-sensitive public→buyer resolution + JSON price parse and is **staged as Batch 4b** — not shipped here. The legacy `VehicleOffer`/`DealerOfferSubmission`/`BuyerOfferReview` models remain read-only intake history.

**Tests:** `lib/services/offer/__tests__/concierge-offer.test.ts` (NEW, 7 cases) — honest OTD, deposit-optional CLOSED auction, idempotency, outside-dealer identity, input validation (non-positive OTD, financing-without-terms), financing terms recorded, and the P2002 concurrency race. Wired via new `test:offer` script into `test:all`.

**Verification (this session):** `pnpm typecheck` ✓ · `pnpm lint` ✓ (0 errors) · `pnpm test:coverage-check` ✓ (160/160 reachable) · `pnpm test:all` ✓ (all suites `# fail 0`, incl. new `test:offer`) · `pnpm build` ✓. Independent second review performed.

**NOT done in Batch 4 (Batch 6 / 4b):** Stripe deposit/refund, DocuSign envelopes, financing submission, deal-completion downstream, service-fee mechanics (Batch 6); System B public→buyer auto-conversion (Batch 4b). No production mutation, no real comms, no migration applied to production.

**Independent review — findings & resolution.** A second, independent review confirmed the core convergence sound (idempotency, P2002 concurrency, honest pricing, nullable-`depositId` reader coverage, migration safety, FS-I keying) and surfaced downstream-surface issues. **Fixed in this batch (both caused by the convergence):**
- **M1 — placeholder dealer name leaked to buyers.** Now that a concierge deal points at a real `Offer` whose dealer is the Outside-Dealer placeholder, the buyer deal/receipt/contract pages showed the internal string "AutoLenis Outside Dealer System" (previously `deal.offer === null` → "AutoLenis Concierge"). Added `lib/services/offer/dealer-display.ts::buyerFacingDealerName` (registered dealer → its name; placeholder + external seller → external name; else "AutoLenis Concierge") and wired it into `buyer/deal`, `buyer/deal/[dealId]/receipt`, and `buyer/contracts/[contractId]` (also suppressing placeholder city/state/tier). Never exposes the placeholder identity.
- **M2 — `BestPriceCalculationLog` written on every buyer GET poll.** The audit write lived in `rankOffers`, called by the polled buyer best-price GET → unbounded row growth + a floating promise that may not flush on serverless. Made the write **opt-in** (`rankOffers(auctionId, termMonths, { persistLog: true })`, awaited); only the terminal callers persist (`processAuctionClose`, admin explicit re-run). The buyer GET no longer writes.

**Deferred (pre-existing, not caused by Batch 4; production has 0 deals so no live exposure):** L1 — a concierge conversion failure leaves the `VehicleRequestOffer` `ACCEPTED` with no Deal and no retry sweep (honest "team will finalize" degradation preserved from the legacy path); a reconciler scanning `OFFER_ACCEPTED`-without-`Deal` and re-running the idempotent `convertConciergeOfferToDeal` is a **Batch 4b** item. L2 — `AdminDealTabs.DealRecord.offer` is typed non-null and read without `?.`, so a legacy null-offer deal (none exist in production) could throw in the admin view; a defensive guard is queued for the admin-deal surface pass. L3 — System C carries no dealer identity, so a concierge offer's `externalDealerName` is empty by nature; the buyer-facing symptom is already resolved by the M1 helper's "AutoLenis Concierge" fallback.
