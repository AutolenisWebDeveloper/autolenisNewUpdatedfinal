...
- **FS-A — $99 deposit refundability (decision #3, consumer/legal).** ~50 strings promised the $99 is fully/auto-refundable while the backend retains it. The *direction* of alignment is a business/legal decision. **→ RESOLVED, see §26** (owner chose "refundable on request, manually reviewed"; copy aligned).
...
## 26. FS-A — $99 deposit refundability (owner decision RESOLVED = refundable on request, manual)

**Owner decision (§16 #3):** the $99 Auction Access Deposit is **refundable ON REQUEST, manually reviewed** by the team — NOT automatic, NOT guaranteed, NOT "fully/100% refundable", NOT a "money-back guarantee", NOT "refunded within N business days". This matches what the code already does (retain by default; a manual admin refund route exists) and the honest `DeclineAuctionButton` wording. No refund *money path* was built (that would be the "automatic refund on no-offer" option the owner did not choose); this is a **copy-alignment** change only.

**Delivered — ~40 files, ~84 strings rewritten** from the false auto/full-refund promise to the truthful "you can request a refund — our team reviews every request":
- **Buyer portal:** `buyer/deposit`, `deposit/success`, `fee`, `billing`, `dashboard`, `deal/payment`, `requests/new`, `requests/[requestId]`, `auction/[auctionId]` (closed/no-offer state), `auth/signup`, `auth/accept-terms`; `PreIntelligencePanel` (the "$99 Refund Guarantee → Not satisfied? Request a refund" block), `VehicleDetailPanel`.
- **Comms/AI:** `deposit-confirmation` email, `zura-voice`/`zura-knowledge`/`context-builder` (dropped the "refundable" adjective / truthful manual-review answers), `nudge` + acquisition SMS, two admin buyer-notification bodies.
- **Public marketing:** home, how-it-works, pricing, for-buyers, about, trust, compare, thank-you, testimonials, inventory, `cars/*`, `faq` (page + layout JSON-LD), `lp/[campaign]`, OG image, SEO metadata/badges, and **`legal/terms`** ("non-refundable except at AutoLenis's discretion: if no competitive offer is received you may request a refund, reviewed and, if approved, returned to the original payment method" — legal should review final wording).

**Compliance rule untouched:** the new phrasing avoids the word "refundable" (the `pricing_language` rule flags "refundable $99/deposit" as an over-promise), so `lib/content/compliance.ts` + its 54-case suite stay green. Only truthful FAQ *questions* ("Is the Auction Access Fee refundable?") retain the word, with answers now stating "isn't automatically refunded, but you can request a refund." The $499 concierge-fee copy, insurance/trade-in refunds, and the factual admin post-refund receipt were left untouched.

**Independent review — findings & resolution.** The review confirmed criteria 2–6 clean (no $499/insurance copy touched, interpolations/JSX intact, phrasing consistent, legal-terms internally consistent, AI/voice truthful) and caught **three residual auto-refund over-promises the sweep missed** — all fixed: `faq/layout.tsx:19` (JSON-LD "refunded in full within 3 business days"), `buyer/auction/[auctionId]/page.tsx:75` ("will be refunded within 3 business days"), `auth/signup/SignUpClient.tsx:196` ("walk away with a full refund"). An exhaustive per-phrase re-sweep is now clean. Re-ran all gates green: `pnpm typecheck` ✓ · `pnpm lint` ✓ (0 errors) · `pnpm test:content` ✓ (54) · `pnpm test:all` ✓ (42 suites) · `pnpm build` ✓.

**FOLLOW-UP flagged (separate owner decision — NOT resolved here):** the platform ships **two contradictory pricing narratives**. Model X (backend `Deposit` model + `legal/terms` + buyer portal + this FS-A copy): a **"deposit," refundable on request, credited toward the $499** (net $400 — a code fact via `PREMIUM_FEE_REMAINING_CENTS`). Model Y (SEO `entity-graph`/`metadata`/`jsonld`, the `/lp/[campaign]` pages, `zura-knowledge`, and the `lib/content/compliance` rule): a **"one-time non-refundable Auction Access Fee, not a deposit, not credited toward purchase."** These disagree on all three of naming (deposit vs fee), crediting (toward the $499 vs not), and now refundability (on-request vs non-refundable). FS-A resolved only the *refund-promise* half; reconciling the deposit-vs-fee naming + credited-toward-purchase stance (and whether the SEO/`/lp`/compliance "non-refundable" language should soften to "refund on request") is a **branding/pricing decision for the owner**, out of FS-A's scope.
