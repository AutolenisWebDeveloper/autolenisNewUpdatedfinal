...
**Verification (this session):** `pnpm typecheck` ✓ · `pnpm lint` ✓ (0 errors) · `pnpm test:coverage-check` ✓ (165 reachable) · `pnpm test:all` ✓ (42 suites, `# fail 0`; webhooks suite unaffected) · `pnpm build` ✓ (new cron route registered). Independent second review performed.
